...
(\emph{European\textsubscript{GEN} Fund\textsubscript{GEN} Development\textsubscript{GEN} Regional\textsubscript{GEN}}) consists of two basic genitive noun phrases, of which only the first one (``European Fund'') needs to be normalized, whereas the 
...
detection of inner syntactic structure of complex proper names is not trivial~\cite{DBLP:conf/acl/Radziszewski13}, 
and thus complicates the process of automated lemmatization. Complex person name declension 
paradigms~\cite{journals/ir/PiskorskiWS09} add another level of complexity.
  ...
  \begin{center}
    \begin{footnotesize}
      ...
      \begin{tabular}{lll}
        ...
        Russian & \textcyr{Европейской комиссией} & \textcyr{Европейская комиссия} \\
        Slovene & Evropske komisije & Evropska komisija \\
        Slovak & Európskej komisie & Európska komisia \\
        Ukrainian & \textcyr{Європейської Комісії} & \textcyr{Європейська Комісія} \\
        \bottomrule
      \end{tabular}
    \end{footnotesize}
  \end{center}
  \caption{Inflected (genitive) forms of the name ``\textit{European
      Commission}'' found in test data.} 
  \label{tab:inflected}
\end{table}
}
...
\resizebox{1.02\linewidth}{!}{
      \begin{tabular}{lll}
        ...
        ru & \textcyr{Европейской комиссией} & \textcyr{Европейская комиссия} \\
        sl & Evropske komisije & Evropska komisija \\
        sk & Európskej komisie & Európska komisia \\
        ua & \textcyr{Європейської Комісії} & \textcyr{Європейська Комісія} \\
        \bottomrule
      \end{tabular}
}
    \end{footnotesize}
