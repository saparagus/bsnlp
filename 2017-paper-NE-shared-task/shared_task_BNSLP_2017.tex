...
\todo{add RW for other languages?}There were some interest in NE for Slavic languages. A corpus with
...
Czech, Polish, Russian, Slovene, and Ukrainian\todo{Ukranian: native or non-native?} were done by native speakers, whereas
annotation for Slovak and Ukrainian were made by native speakers of Czech, capable 
of understanding the former\todo{fix: "former"} language. The cross-language alignment of the entity identifiers was
...
test datasets. \alert{A case in point is a document in Croatian that contained the
	...
	županija}''---a contracted version of three named entities
	(``\textit{Zagrebačka županija}'', ``\textit{Sisačko-Moslavačka
županija}'', and ``\textit{Karlovačka županija}'') expressed using a head noun with three coordinate modifiers.}
